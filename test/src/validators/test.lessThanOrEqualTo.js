...
describe("is.lessThanOrEqualTo", () => {
	//! TRUE
	it(`can tell that 2 is lesser than or equal to 3`, () => {
		assert.isTrue(is.lessThanOrEqualTo(2, 3));
	});
	it(`can tell that a is lesser than or equal to abc`, () => {
		assert.isTrue(is.lessThanOrEqualTo("a", "abc"));
	});
    it(`can tell that 3 is lesser than or equal to 3`, () => {
		assert.isTrue(is.lessThanOrEqualTo(3, 3));
    });
    it(`can tell that abc is lesser than or equal to abc`, () => {
		assert.isTrue(is.lessThanOrEqualTo("abc", "abc"));
	});
    //! FALSE
    it(`can tell that Infinity is NOT lesser than or equal to anything`, () => {
		assert.isFalse(is.lessThanOrEqualTo(Infinity, 0));
	});
	it(`can tell that anything is NOT lesser than or equal to Infinity`, () => {
		assert.isFalse(is.lessThanOrEqualTo(0, Infinity));
    });
	it(`can tell that 3 is NOT lesser than or equal to 2`, () => {
		assert.isFalse(is.lessThanOrEqualTo(3, 2));
	});
	it(`can tell that abc is NOT lesser than or equal to a`, () => {
