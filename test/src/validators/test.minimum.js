...
describe("is.minimum", () => {
	//! TRUE
	it(`can tell that 1 is minimum of [3,2,1]`, () => {
		assert.isTrue(is.minimum(1, [3, 2, 1]));
	});
	it(`can tell that 0 is minimum of [1,2,3]`, () => {
		assert.isTrue(is.minimum(0, [1, 2, 3]));
	});
	it(`can tell that a is minimum of [a,b,c]`, () => {
		assert.isTrue(is.minimum("a", ["a", "b", "c"]));
	});
	//! FALSE
	it(`can tell that c is NOT minimum of [a,b,c]`, () => {
		assert.isFalse(is.minimum("c", ["a", "b", "c"]));
	});
	it(`can tell that 2 is NOT minimum of [1,2,3]`, () => {
