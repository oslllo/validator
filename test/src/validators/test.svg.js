...
describe("is.svg", () => {
	...
	valid.forEach((svg) => {
		it(`can validate that ${svg} is a valid SVG`, () => {
			assert.isTrue(is.svg(svg));
		});
	});
	var invalid = [
		"this is not svg, but it mentions <svg> tags",
	...
	invalid.forEach((svg) => {
		it(`can validate that ${svg} is NOT a valid SVG`, () => {
			assert.isFalse(is.svg(svg));
		});
	});
	inputs.invalid([]).forEach((invalid) => {
		it(`throws with invalid SVG input ${invalid.description}`, () => {
			assert.throws(() => is.svg(invalid), TypeError);
		});
	});
});
