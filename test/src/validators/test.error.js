...
describe("is.error", () => {
    ...
    inputs.valid(data).forEach((valid) => {
        it(`can validate that ${valid.description} is an Error`, () => {
            assert.isTrue(is.error(valid.data));
        });
    });
    inputs.invalid(data).forEach((invalid) => {
        it(`can validate that ${invalid.description} is NOT an Error`, () => {
    ...
    it("can validate that object with Error's toString method is NOT an error", () => {
